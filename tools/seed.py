#!/usr/bin/env python3
"""Synthetic cadastre generator for the Land Stack demo AOI (CONTRACTS §10).

Builds a deterministic, believable peri-urban cadastre inside a *real* bounding box on the
Mangalagiri fringe (Guntur district, AP): a road network (synthetic, or OpenStreetMap with
``--osm`` when Overpass is reachable), blocks polygonised from the roads, parcels carved from the
blocks (small urban plots near the town corner, agricultural holdings further out), master-plan
zones, restriction zones, projects, water lines, one record per department in that department's
own vocabulary, a few buildings with per-floor units for the 3D preview, and Sentinel-2 index
values with planted change alerts. Six story parcels (123/4, 124, 125/2, 126, 127/1, 128) are
guaranteed to exist with the traits the demo script relies on.
...
AOI_NAME = "Mangalagiri (R) — demo"
STATE, DISTRICT, TALUK, VILLAGE = "AP", "Guntur", "Mangalagiri", "Mangalagiri (R)"
SRO_CODE = "GNT-02"
DEFAULT_SEED = 42
FLOOR_HEIGHT_M = 3.2
...
# Story parcels: survey_no → (key, land_use, subdivision group size, index of the story parcel in the group)
STORY = {
...
class Parcel:
    ...
    story: str | None = None             # key from STORY
...
@dataclass
class Frame:
    """Everything the seed produces, in insertion-ready Python structures."""
...
def osm_roads(aoi: Polygon, timeout_s: float = 20.0) -> list[Road] | None:
...
def pick_story_parcels(parcels: list[Parcel], model: SizeModel, rng: random.Random) -> dict[str, Parcel]:
    """Choose the six story parcels: 123/4 and 124 adjacent near the AOI centre, the rest nearby."""
    centre = model.centre
    geoms = [p.geom_utm for p in parcels]
    tree = STRtree(geoms)
    ...
    _, i_agri, j_res = best
    chosen: dict[str, Parcel] = {"124": parcels[i_agri], "123/4": parcels[j_res]}

    used = {i_agri, j_res}
    remaining = [("125/2", "semi"), ("126", "semi"), ("127/1", "agri"), ("128", "semi")]
...
def assign_survey_numbers(parcels: list[Parcel], story: dict[str, Parcel], rng: random.Random) -> None:
    ...
    # Base numbers: sequential in traversal order, anchored so 123/4's group is 123.
    anchor_idx = next(i for i, g in enumerate(ordered_groups) if g[1] == "123/4")
    non_story_before = sum(1 for g in ordered_groups[:anchor_idx] if g[1] is None)
    number = max(1, 123 - non_story_before)
    reserved = {123, 124, 125, 126, 127, 128}
    for members, story_survey in ordered_groups:
        if story_survey is not None:
            base = int(story_survey.split("/")[0])
            _, _, size, idx = STORY[story_survey]
...
def make_reference_layers(frame: Frame, rng: random.Random) -> None:
    ...
    frame.restriction_zones = [
        {"kind": "flood", "name": "Krishna floodplain (indicative)", "geom_utm": flood},
        {"kind": "heritage", "name": "Temple precinct heritage buffer (indicative)", "geom_utm": heritage},
        {"kind": "eco_sensitive", "name": "Kondaveeti vagu wetland patch (indicative)", "geom_utm": eco},
    ]
    ...
    frame.projects = [
        {"name": "Outer Ring Road link (proposed)", "kind": "road", "status": "proposed", "geom": to_wgs(ring_road)},
        {"name": "Amaravati–Vijayawada metro corridor (indicative)", "kind": "metro", "status": "proposed",
         "geom": to_wgs(as_multipolygon(metro))},
    ]
...
class Names:
    """Indian names: 70 % curated Telugu (Guntur) names, 30 % Faker en_IN, seeded for determinism."""
    ...
    def person(self, rng: random.Random) -> tuple[str, str]:
        """Return ``(owner_name, father_name)`` (father shares the surname, as on AP pattadar passbooks)."""
        female = rng.random() < 0.35
        if rng.random() < 0.7:
            surname = rng.choice(TELUGU_SURNAME)
            first = rng.choice(TELUGU_FEMALE if female else TELUGU_MALE)
            father = f"{rng.choice(TELUGU_MALE)} {surname}"
...
def make_department_records(frame: Frame, rng: random.Random, names: Names) -> None:
    ...
    for p in frame.parcels:
        ...
        if govt:
            owner, father, otype = rng.choice(["Government of Andhra Pradesh", "Mangalagiri Municipality",
                                                "Gram Panchayat, Mangalagiri (R)"]), None, "govt"
        ...
        if not govt:
            ...
            frame.property_tax.append({
                "assessment_no": f"MGL-{assess_seq:06d}", "ulpin": p.ulpin, "annual_demand": demand,
        ...
        if registered:
            ...
            for k in range(n_deeds):
                ...
                frame.deeds.append({
                    ...
                    "extent_sqm": extent, "registered_on": reg_on, "sro_code": SRO_CODE,
                })
        ...
        if p.land_use in {"residential", "commercial"} and p.story != "clean_residential" and rng.random() < 0.35:
            ...
            frame.permissions.append({
                ...
                "conditions": "Setbacks as per APBR 2017; rainwater harvesting mandatory." if status == "approved" else None,
                "application_id": None,
            })
        ...
        if p.story == "disputed" or (p.story is None and rng.random() < 0.04):
            ...
            frame.disputes.append({
                "case_no": f"OS {case_seq}/{filed.year}", "ulpin": p.ulpin, "court": rng.choice(COURTS),
...
def make_buildings(frame: Frame, rng: random.Random, names: Names) -> None:
    ...
    for p, (floors, upf, nm) in zip(commercial[:2], [(4, 3, "Sri Lakshmi Complex"), (5, 4, "Padmavathi Towers")]):
        picks.append((p, floors, upf, nm))
    for p, (floors, upf, nm) in zip(residential[:2], [(3, 2, "Sai Residency"), (3, 2, "Gokul Apartments")]):
        picks.append((p, floors, upf, nm))

    for bid, (p, floors, upf, name) in enumerate(picks, start=1):
...
# Orchestration
# ---------------------------------------------------------------------------
def build_frame(seed: int = DEFAULT_SEED, use_osm: bool = False, verbose: bool = True) -> Frame:
    """Generate the full synthetic cadastre deterministically for ``seed``."""
    log = print if verbose else (lambda *a, **k: None)
    t0 = time.perf_counter()
    rng = random.Random(seed)
    names = Names(seed)
    aoi = aoi_polygon_utm()
    frame = Frame(seed=seed, aoi_utm=aoi)

    roads = osm_roads(aoi) if use_osm else None
    frame.roads = roads or synthetic_roads(aoi, rng)
    log(f"roads          : {len(frame.roads)} ({'osm' if roads else 'synthetic'})")

    model = SizeModel(aoi, frame.roads, rng)
    frame.blocks = make_blocks(aoi, frame.roads)
    log(f"blocks         : {len(frame.blocks)}")
    frame.parcels = make_parcels(frame.blocks, model, rng)
    pairs = assert_no_overlaps(frame.parcels)
    log(f"parcels        : {len(frame.parcels)} (no overlaps > 1 m² across {pairs} touching pairs)")
    if not (500 <= len(frame.parcels) <= 1200):
        raise AssertionError(f"parcel count {len(frame.parcels)} outside 500–1200; adjust SizeModel")

    frame.zones = make_zones(aoi, frame.roads, model)
    assign_zones_and_land_use(frame.parcels, frame.zones, rng)
    frame.story = pick_story_parcels(frame.parcels, model, rng)
    assign_survey_numbers(frame.parcels, frame.story, rng)
    ...
    log(f"frame built in {time.perf_counter() - t0:.1f}s")
    return frame


def summary(frame: Frame) -> str:
    counts = [
        ("landstack.parcels", len(frame.parcels)), ("landstack.buildings", len(frame.buildings)),
        ("landstack.units", len(frame.units)), ("landstack.users", len(frame.users)),
        ("landstack.applications", len(frame.applications)), ("landstack.alerts", len(frame.alerts)),
        ("landstack.audit_log", len(frame.audit_log)), ("dept_revenue.ror", len(frame.ror)),
        ("dept_revenue.mutations", len(frame.mutations)), ("dept_registration.deeds", len(frame.deeds)),
        ("dept_registration.encumbrances", len(frame.encumbrances)), ("dept_planning.zones", len(frame.zones)),
        ("dept_planning.building_permissions", len(frame.permissions)), ("dept_fiscal.property_tax", len(frame.property_tax)),
        ("dept_fiscal.valuation", len(frame.valuation)), ("dept_legal.disputes", len(frame.disputes)),
        ("dept_utilities.connections", len(frame.connections)), ("gis.roads", len(frame.roads)),
        ("gis.water_lines", len(frame.water_lines)), ("gis.restriction_zones", len(frame.restriction_zones)),
        ("gis.projects", len(frame.projects)), ("gis.village_boundary", 1), ("gis.s2_change", len(frame.s2_change)),
    ]
    lines = ["", f"{'table':38} rows", "-" * 44]
    lines += [f"{name:38} {n:5d}" for name, n in counts]
    lu = Counter(p.land_use for p in frame.parcels)
    zc = Counter(p.zone_code for p in frame.parcels)
    areas = sorted(p.area_sqm for p in frame.parcels)
    lines += ["", f"land_use : {dict(lu)}", f"zones    : {dict(zc)}",
              f"area m²  : min {areas[0]:.0f} · median {areas[len(areas) // 2]:.0f} · max {areas[-1]:.0f}",
              f"changed  : {sum(1 for s in frame.s2_change if s['label'] != 'no_significant_change')} parcels with S2 change labels",
              "", f"{'story':8} {'ulpin':16} {'land_use':13} {'area':>8}  owner"]
    ror = {r["ulpin"]: r for r in frame.ror}
    for survey in STORY:
        p = frame.story[survey]
        lines.append(f"{survey:8} {p.ulpin:16} {p.land_use:13} {p.area_sqm:8.0f}  {ror[p.ulpin]['owner_name']}")
    return "\n".join(lines)
...
def write_geojson(frame: Frame, out_dir: Path) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    ror = {r["ulpin"]: r for r in frame.ror}
    s2 = {s["ulpin"]: s for s in frame.s2_change}
    parcels = [_feature(p.geom, {
        "ulpin": p.ulpin, "survey_no": p.survey_no, "sub_division": p.sub_division, "village": VILLAGE,
    ...
    (out_dir / "village.geojson").write_text(json.dumps(_fc([_feature(to_wgs(as_multipolygon(frame.aoi_utm)), {"name": AOI_NAME})]), separators=(",", ":")))

    story = {
        "seed": frame.seed, "aoi_bbox": list(AOI_BBOX), "village": VILLAGE,
        "parcels": [{
            "survey_no": survey, "key": STORY[survey][0], "ulpin": p.ulpin, "land_use": p.land_use,
        ...
        } for survey, p in ((s, frame.story[s]) for s in STORY)],
...
def write_department_demo_tables(cur, frame: Frame) -> dict[str, int]:
    ...
    n["dept_revenue.ror"] = _insert(cur, "dept_revenue.ror",
        ...
        [(r["khata_no"], r["ulpin"], r["survey_no"], r["owner_name"], r["father_name"], r["ownership_type"], r["extent_sqm"],
          r["classification"], _j(r["mutation_history"]), r["updated_at"]) for r in frame.ror], {"mutation_history": JSONB})
    ...
    n["dept_registration.encumbrances"] = _insert(cur, "dept_registration.encumbrances",
        ...
        [(e["ulpin"], e["kind"], e["holder"], e["amount"], e["from_date"], e["to_date"], e["active"]) for e in frame.encumbrances])
    n["dept_planning.building_permissions"] = _insert(cur, "dept_planning.building_permissions",
        ...
        [(b["permit_no"], b["ulpin"], b["status"], b["floors"], b["built_up_sqm"], b["applied_on"], b["approved_on"], b["conditions"], b["application_id"])
         for b in frame.permissions])
    return n


def write_mutable(cur, frame: Frame) -> dict[str, int]:
    ...
    n["landstack.applications"] = _insert(cur, "landstack.applications",
        ...
        [(a["id"], a["ulpin"], a["type"], a["applicant_uid"], a["applicant_name"], a["status"], _j(a["payload"]),
          a["assigned_department"], a["created_at"], a["updated_at"]) for a in frame.applications], {"payload": JSONB})
    n["landstack.alerts"] = _insert(cur, "landstack.alerts",
        ["ulpin", "kind", "severity", "title", "detail", "status", "created_at"],
        [(a["ulpin"], a["kind"], a["severity"], a["title"], _j(a["detail"]), a["status"], a["created_at"]) for a in frame.alerts],
        {"detail": JSONB})
    n["landstack.audit_log"] = _insert(cur, "landstack.audit_log",
        ...
        [(a["ts"], a["actor_uid"], a["actor_name"], a["actor_role"], a["action"], a["entity_type"], a["entity_id"], a["ulpin"],
         ...
         for a in frame.audit_log], {"before": JSONB, "after": JSONB})
    n["dept_revenue.mutations"] = _insert(cur, "dept_revenue.mutations",
        ["ulpin", "from_owner", "to_owner", "reason", "application_id", "created_at"],
        [(m["ulpin"], m["from_owner"], m["to_owner"], m["reason"], m["application_id"], m["created_at"]) for m in frame.mutations])
    cur.execute("SELECT setval('landstack.application_id_seq', %s, true)", (max(1, len(frame.applications)),))
    return n


def write_db(conn, frame: Frame, only_mutable: bool = False) -> dict[str, int]:
    """Load the frame into PostgreSQL inside one transaction. Returns per-table row counts."""
    with conn.cursor() as cur:
        if only_mutable:
            n = write_department_demo_tables(cur, frame)
            n.update(write_mutable(cur, frame))
            conn.commit()
            return n
        ...
        n["landstack.parcels"] = _insert(cur, "landstack.parcels",
            ...
            [(p.ulpin, STATE, DISTRICT, TALUK, VILLAGE, p.survey_no, p.sub_division, _wkt(p.geom), p.area_sqm, p.land_use, p.zone_code,
              _j({"story": p.story} if p.story else {})) for p in frame.parcels], {"geom": MULTI, "status_flags": JSONB})
        n["landstack.buildings"] = _insert(cur, "landstack.buildings",
            ["id", "ulpin", "footprint", "floors", "height_m", "name"],
            [(b["id"], b["ulpin"], _wkt(b["footprint"]), b["floors"], b["height_m"], b["name"]) for b in frame.buildings], {"footprint": MULTI})
        cur.execute("SELECT setval('landstack.buildings_id_seq', %s, true)", (max(1, len(frame.buildings)),))
        n["landstack.units"] = _insert(cur, "landstack.units",
            ...
            [(u["building_id"], u["ulpin"], u["ulpin_3d"], u["floor"], u["unit_no"], _wkt(u["geom"]), u["base_m"], u["height_m"], u["owner_name"])
             for u in frame.units], {"geom": GEOM})
        n["landstack.users"] = _insert(cur, "landstack.users", ["uid", "email", "name", "role", "department"],
            [(u["uid"], u["email"], u["name"], u["role"], u["department"]) for u in frame.users])

        n["dept_planning.zones"] = _insert(cur, "dept_planning.zones", ["zone_code", "name", "permissible_uses", "geom"],
            [(z["zone_code"], z["name"], z["permissible_uses"], _wkt(z["geom"])) for z in frame.zones], {"geom": MULTI})
        n["dept_fiscal.property_tax"] = _insert(cur, "dept_fiscal.property_tax",
            ...
            [(t["assessment_no"], t["ulpin"], t["annual_demand"], t["paid_till"], t["arrears"], t["last_paid_on"]) for t in frame.property_tax])
        n["dept_fiscal.valuation"] = _insert(cur, "dept_fiscal.valuation", ["ulpin", "guideline_value_per_sqm", "effective_from"],
            [(v["ulpin"], v["guideline_value_per_sqm"], v["effective_from"]) for v in frame.valuation])
        n["dept_legal.disputes"] = _insert(cur, "dept_legal.disputes",
            ["case_no", "ulpin", "court", "nature", "filed_on", "status", "next_hearing"],
            [(d["case_no"], d["ulpin"], d["court"], d["nature"], d["filed_on"], d["status"], d["next_hearing"]) for d in frame.disputes])
        n["dept_utilities.connections"] = _insert(cur, "dept_utilities.connections",
            ...
            [(c["ulpin"], c["water"], c["electricity"], c["sewer"], c["road_access_m"], c["nearest_road_class"]) for c in frame.connections])

        n["gis.roads"] = _insert(cur, "gis.roads", ["name", "road_class", "width_m", "source", "geom"],
            [(r.name, r.road_class, r.width_m, r.source, _wkt(to_wgs(r.geom_utm))) for r in frame.roads], {"geom": GEOM})
        n["gis.water_lines"] = _insert(cur, "gis.water_lines", ["name", "kind", "geom"],
            [(w["name"], w["kind"], _wkt(w["geom"])) for w in frame.water_lines], {"geom": GEOM})
        n["gis.restriction_zones"] = _insert(cur, "gis.restriction_zones", ["kind", "name", "geom"],
            [(z["kind"], z["name"], _wkt(z["geom"])) for z in frame.restriction_zones], {"geom": MULTI})
        n["gis.projects"] = _insert(cur, "gis.projects", ["name", "kind", "status", "geom"],
            [(p["name"], p["kind"], p["status"], _wkt(p["geom"])) for p in frame.projects], {"geom": GEOM})
        n["gis.village_boundary"] = _insert(cur, "gis.village_boundary", ["name", "geom"],
            [(AOI_NAME, _wkt(to_wgs(as_multipolygon(frame.aoi_utm))))], {"geom": MULTI})
        n.update(write_department_demo_tables(cur, frame))
...
def main(argv: list[str] | None = None) -> int:
    ...
    frame = build_frame(seed=args.seed, use_osm=args.osm)
    if args.geojson_out:
        write_geojson(frame, args.geojson_out)
    print(summary(frame))
    ...
    with connect(url) as conn:
        counts = write_db(conn, frame, only_mutable=args.only_mutable)
