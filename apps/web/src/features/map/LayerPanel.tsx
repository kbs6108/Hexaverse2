...
export function LayerPanel() {
  ...
  if (!layerPanelOpen) {
    return (
      <button
        ...
        className="absolute top-24 left-4 z-40 flex items-center gap-2 rounded-2xl border border-[#D5D2C7] bg-[#F4F1E7]/85 px-4 py-2.5 text-sm font-black tracking-tight text-[#18231F] shadow-[0_12px_40px_rgba(24,35,31,0.08)] backdrop-blur-2xl hover:bg-[#F4F1E7] transition-all cursor-pointer"
      >
  ...
  return (
    <aside aria-label="Layers" className="absolute top-24 left-4 z-40 w-[320px] max-h-[calc(100vh-7rem)] overflow-y-auto bg-[#F4F1E7]/85 backdrop-blur-2xl border border-[#D5D2C7] shadow-[0_12px_40px_rgba(24,35,31,0.08)] rounded-2xl p-5 text-[#18231F] scroll-thin flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between pb-3 border-b border-[#D5D2C7]/60">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[#18231F] font-black text-base tracking-tight">Layers</h2>
        </div>
        <button
          type="button"
        ...
        <div className="space-y-1.5">
          <Field label={<span className="text-xs font-semibold text-[#18231F]">Colour parcels by</span>} htmlFor="colour-by">
            <select
              ...
              className="w-full bg-[#E9E5D8]/70 border border-[#D5D2C7] rounded-xl px-3 py-2 text-xs font-semibold text-[#18231F] focus:ring-2 focus:ring-[#176B52]/40 outline-none cursor-pointer"
            >
          ...
          <p className="mt-2 flex items-center gap-1 text-[11px] text-[#4B5345]">
            <TriangleAlert size={12} className="text-[#9A6B12] shrink-0" /> Disputed parcels are always hatched, change alerts always carry a marker.
          </p>
        </div>
      ...
      {/* Basemap Switcher + 3D */}
      <div className="flex flex-col gap-2.5 border-t border-[#D5D2C7]/60 pt-3 mt-3">
        <div role="radiogroup" aria-label="Basemap" className="bg-[#E9E5D8]/80 p-1 rounded-xl border border-[#D5D2C7]/70 flex gap-1">
          <button
            ...
            className={clsx(
              'flex-1 text-center transition-colors cursor-pointer',
              basemap === 'streets'
                ? 'bg-[#23483A] text-[#F4F1E7] font-bold text-xs rounded-lg py-1 px-3 shadow-xs'
                : 'text-[#6F7768] hover:text-[#18231F] text-xs font-medium py-1 px-3'
            )}
          >
            Streets
          </button>
          <button
            ...
            className={clsx(
              'flex-1 flex items-center justify-center gap-1 transition-colors cursor-pointer',
              basemap === 'imagery'
                ? 'bg-[#23483A] text-[#F4F1E7] font-bold text-xs rounded-lg py-1 px-3 shadow-xs'
                : 'text-[#6F7768] hover:text-[#18231F] text-xs font-medium py-1 px-3'
            )}
          ...
          <Checkbox
            label={
              <span className="flex items-center gap-1.5">
                <Box size={14} className="text-[#176B52]" /> 3D units · preview
              </span>
            }
...
function Tier({ title, defaultOpen, open: controlledOpen, onToggle, children }: {
  ...
  return (
    <section className="border-t border-[#D5D2C7]/60 pt-3 mt-3 first:border-t-0 first:pt-0 first:mt-0">
      <button
        type="button"
        aria-expanded={open}
      ...
      >
        {title}
        <ChevronDown size={14} className={clsx('transition-transform text-[#6F7768]', open && 'rotate-180')} />
      </button>
