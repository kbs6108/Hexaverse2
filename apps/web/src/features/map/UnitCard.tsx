...
export function UnitCard({ unit, onClose }: { unit: UnitInfo; onClose: () => void }) {
  ...
  return (
    <div className="absolute right-3 top-24 z-20 w-72 rounded-2xl border border-line bg-panel/95 text-ink shadow-elevated backdrop-blur-xl">
      <div className="flex items-center gap-2 border-b border-line px-3 py-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate font-mono text-[12px] font-semibold text-ink">{unit.ulpin_3d ?? '—'}</p>
        </div>
        <button type="button" aria-label="Close unit card" onClick={onClose} className="ml-auto rounded-lg p-1 text-ink-3 hover:bg-ground-2 hover:text-ink transition-colors cursor-pointer">
          <X size={14} />
        </button>
      </div>
      <dl className="grid grid-cols-2 gap-x-3 gap-y-1.5 px-3 py-2.5">
        {rows.map(([k, v]) => (
          <div key={k} className={k === 'Occupant (record)' ? 'col-span-2' : ''}>
            ...
            <dd className="text-[12.5px] text-ink font-medium">{v}</dd>
          </div>
        ))}
      </dl>
    </div>
